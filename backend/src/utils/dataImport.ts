 ...
 */
function validateCustomerData(data: ImportCustomerData): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export async function linkRelationshipManagers(): Promise<{
  ...
  const customers = await prisma.customer.findMany({
    where: {
      ...
      relationshipManagerId: null, // Only link those not already linked
    },
  });
    ...
    if (user) {
      await prisma.customer.update({
        ...
        data: {
          ...
          // Optionally clear the temp field: relationshipManagerName: null
        },
      });
      linked++;
