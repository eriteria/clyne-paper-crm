...
async function testDateFilter() {
  try {
    ...
    if (sampleInvoice) {
      ...
      console.log(
        `Sample invoice createdAt UTC: ${sampleInvoice.createdAt.toISOString()}`
      );
      console.log(
        `Is createdAt >= startDate? ${sampleInvoice.createdAt >= startDate}`
      );
      console.log(
        `Is createdAt <= endDate? ${sampleInvoice.createdAt <= endDate}`
      );
    }
